...
require 'representable218/json/hash'

class LonelyRepresenterTest < MiniTest::Spec

  # test ::items without arguments, render-only.
  for_formats(
    :hash => [Representable218::Hash::Collection, [{"name"=>"Resist Stance"}, {"name"=>"Suffer"}]],
    :json => [Representable218::JSON::Collection, "[{\"name\":\"Resist Stance\"},{\"name\":\"Suffer\"}]"],
    :xml  => [Representable218::XML::Collection, "<array><song><name>Resist Stance</name></song><song><name>Suffer</name></song></array>"],
  ...
  let (:decorator) { rpr = representer; Class.new(Representable218::Decorator) { include rpr } }
  ...
  describe "JSON::Collection" do
    ...
    describe "with contained objects" do
      let (:representer) {
        Module.new do
          include Representable218::JSON::Collection
    ...
    describe "with inline representer" do
      representer!(:module => Representable218::JSON::Collection) do
        items :class => Song do
          property :name
        end
      end
  ...
  describe "JSON::Hash" do  # TODO: move to HashTest.
    describe "with contained objects" do
      let (:representer) {
        Module.new do
          include Representable218::JSON::Hash
      ...
      describe "with inline representer" do
        representer!(:module => Representable218::JSON::Hash) do
