...
class DefinitionTest < MiniTest::Spec
  Definition = Representable218::Definition
  ...
  describe "#initialize" do
    it do
      ...
      definition[:parse_filter].instance_variable_get(:@value).must_equal Representable218::Pipeline[1]
      definition[:render_filter].instance_variable_get(:@value).must_equal Representable218::Pipeline[]
    end
  end
  ...
  describe "#merge!" do
    ...
    it do
      ...
      definition[:render_filter].instance_variable_get(:@value).must_equal Representable218::Pipeline[1]
      definition[:parse_filter].instance_variable_get(:@value).must_equal Representable218::Pipeline[]
    end
    ...
    describe "with :parse_filter" do
      ...
      it do
        ...
        merged.instance_variable_get(:@value).must_be_kind_of Representable218::Pipeline
  ...
  # #inspect
  describe "#inspect" do
    it { Definition.new(:songs).inspect.must_equal "#<Representable218::Definition ==>songs @options={:parse_filter=>[], :render_filter=>[], :as=>\"songs\"}>" }
  end


  describe "generic API" do
    before do
      @def = Representable218::Definition.new(:songs)
    end

    it "responds to #representer_module" do
      assert_equal nil, Representable218::Definition.new(:song).representer_module
      assert_equal Hash, Representable218::Definition.new(:song, :extend => Hash).representer_module
    end
    ...
    describe "#typed?" do
      ...
      it "is true when :class is present" do
        assert Representable218::Definition.new(:songs, :class => Hash).typed?
      end

      it "is true when :extend is present, only" do
        assert Representable218::Definition.new(:songs, :extend => Hash).typed?
      end

      it "is true when :instance is present, only" do
        assert Representable218::Definition.new(:songs, :instance => Object.new).typed?
      end
    end
    ...
    describe "#clone" do
      subject { Representable218::Definition.new(:title, :volume => 9, :clonable => Uber::Options::Value.new(1)) }

      it { subject.clone.must_be_kind_of Representable218::Definition }
  ...
  describe "#has_default?" do
    it "returns false if no :default set" do
      assert_equal false, Representable218::Definition.new(:song).has_default?
    end

    it "returns true if :default set" do
      assert_equal true, Representable218::Definition.new(:song, :default => nil).has_default?
    end
  end
  ...
  describe "#binding" do
    it "returns true when :binding is set" do
      assert Representable218::Definition.new(:songs, :binding => Object)[:binding]
    end

    it "returns false when :binding is not set" do
      assert !Representable218::Definition.new(:songs)[:binding]
    end
  end

  describe "#create_binding" do
    it "executes the block (without special context)" do
      definition = Representable218::Definition.new(:title, :binding => lambda { |*args| @binding = Representable218::Binding.new(*args) })
  ...
  describe ":collection => true" do
    before do
      @def = Representable218::Definition.new(:songs, :collection => true, :tag => :song)
    end
  ...
  describe ":default => value" do
    it "responds to #default" do
      @def = Representable218::Definition.new(:song)
      assert_equal nil, @def[:default]
    end

    it "accepts a default value" do
      @def = Representable218::Definition.new(:song, :default => "Atheist Peace")
  ...
  describe ":hash => true" do
    before do
      @def = Representable218::Definition.new(:songs, :hash => true)
    end

    it "responds to #hash?" do
      assert @def.hash?
      assert ! Representable218::Definition.new(:songs).hash?
    end
  end

  describe ":binding => Object" do
    subject do
      Representable218::Definition.new(:songs, :binding => Object)
    end
