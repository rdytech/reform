...
class HashBindingTest < MiniTest::Spec
  ...
  describe "PropertyBinding" do
    describe "#read" do
      before do
        @property = Representable218::Hash::Binding.new(Representable218::Definition.new(:song), nil, nil)
      end
      ...
      it "returns FRAGMENT_NOT_FOUND if not in document" do
        assert_equal Representable218::Binding::FragmentNotFound, @property.read({})
      end
  ...
  describe "CollectionBinding" do
    describe "with plain text items" do
      before do
        @property = Representable218::Hash::Binding::Collection.new(Representable218::Definition.new(:songs, :collection => true), Album.new, nil)
      end
  ...
  describe "HashBinding" do
    describe "with plain text items" do
      before do
        @property = Representable218::Hash::Binding::Hash.new(Representable218::Definition.new(:songs, :hash => true), nil, nil)
      end
    ...
    describe "with objects" do
      before do
        @property = Representable218::Hash::Binding::Hash.new(Representable218::Definition.new(:songs, :hash => true, :class => Song, :extend => SongRepresenter), nil, nil)
      end
