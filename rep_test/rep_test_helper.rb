...
MiniTest::Spec.class_eval do
  ...
  def self.representer!(options={}, &block)
    ...
    format = options[:module] || Representable218::Hash

    let(name) do
      mod = options[:decorator] ? Class.new(Representable218::Decorator) : Module.new
  ...
  module TestMethods
    def representer_for(modules=[Representable218::Hash], &block)
      Module.new do
        extend TestMethods
        include *modules
        module_exec(&block)
