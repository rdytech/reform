...
class ExecContextTest < MiniTest::Spec
  for_formats(
    :hash => [Representable218::Hash, {Song => "Rebel Fate"}, {Song=>"Timing"}],
    ...
    describe "exec_context: :binding" do
      ...
      it { render(song).must_equal_document({Representable218::Hash::Binding => "name"}) }
      it { parse(song, {Representable218::Hash::Binding => "Rebel Fate"}).name.must_equal "Rebel Fate" }
    end
    ...
    describe "Decorator" do
      ...
      describe "exec_context: :decorator" do # this tests if lambdas are run in the right context, if methods are called in the right context and if we can access the represented object.
        ...
        it { render(song).must_equal_document({Representable218::Decorator=>"Timebomb"}) }
        it { parse(song, {Representable218::Decorator=>"Listless"}).name.must_equal "Listless" }
      end
