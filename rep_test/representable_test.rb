...
class RepresentableTest < MiniTest::Spec
  ...
  describe "#property" do
    ...
    it "returns the Definition instance" do
      representer.property(:name).must_be_kind_of Representable218::Definition
    end
  end
  ...
  describe "#update_properties_from" do
    ...
    it "ignores (no-default) properties not present in the incoming document" do
      { Representable218::Hash => [:from_hash, {}],
        Representable218::XML  => [:from_xml,  xml(%{<band/>}).to_s]
  ...
  describe ":extend and :class" do
    ...
    describe ":instance" do
      obj = String.new("Fate")
      mod = Module.new { include Representable218; def from_hash(*); self; end }
      representer! do
    ...
    describe ":decorator" do
      let (:extend_rpr) { Module.new { include Representable218::Hash; collection :songs, :extend => SongRepresenter } }
      let (:decorator_rpr) { Module.new { include Representable218::Hash; collection :songs, :decorator => SongRepresenter } }
    ...
    describe ":binding" do
      representer! do
        class MyBinding < Representable218::Binding
          def write(doc, *args)
    ...
    class SongRepresentation < Representable218::Decorator
      include Representable218::JSON
      property :name
    end
    class AlbumRepresentation < Representable218::Decorator
      include Representable218::JSON
    ...
    describe "::prepare" do
      ...
      describe "module including Representable" do
        it "uses :extend strategy" do
          album_rpr = Module.new { include Representable218::Hash; collection :songs, :class => Song, :extend => SongRepresenter}
  ...
  describe "#use_decorator" do
    ...
    it "uses a Decorator for inline representer" do
      ...
      outer.must_be_kind_of Representable218::Hash
      inner.wont_be_kind_of Representable218::Hash
    end
  end


  # using your own Definition class
  class StaticDefinition < Representable218::Definition
  ...
  module StaticRepresenter
    def self.build_config
      Representable218::Config.new(StaticDefinition)
    end
    include Representable218::Hash
  ...
  class BaseDecorator < Representable218::Decorator
    include Representable218::Hash
    def self.build_config
      Representable218::Config.new(StaticDefinition)
    end
